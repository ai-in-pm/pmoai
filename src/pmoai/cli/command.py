...
from pmoai.cli.plus_api import PlusAPI
...
class PlusAPIMixin:
    def __init__(self, telemetry):
        try:
            ...
            self.plus_api_client = PlusAPI(api_key=self._get_auth_token())
        except Exception:
            self._deploy_signup_error_span = telemetry.deploy_signup_error_span()
            console.print(
                "Please sign up/login to PMOAI+ before using the CLI.",
                style="bold red",
            )
            console.print("Run 'pmoai signup' to sign up/login.", style="bold green")
            raise SystemExit
