...
class AgentTestCase(BaseModel):
    ...
    input: Dict[str, Any] = Field(description="The input for the test case.")
...
class AgentTestResult(BaseModel):
    """Represents the result of an agent test."""
    
    test_case: AgentTestCase = Field(description="The test case that was executed.")
...
class AgentTester(BaseModel):
    """Tests agents with various test cases."""
    
    def test_agent(
        self, agent: Agent, test_cases: List[AgentTestCase]
        ...
        for test_case in test_cases:
            result = self._run_test_case(agent, test_case)
            results.append(result)
        
        return results
    
    def _run_test_case(self, agent: Agent, test_case: AgentTestCase) -> AgentTestResult:
        ...
        task = Task(
            ...
            expected_output="The output of the test case.",
            agent=agent,
        )
        ...
        try:
            result = task.execute(inputs=test_case.input)
    ...
    def _check_test_result(self, test_case: AgentTestCase, actual_output: Any) -> bool:
    ...
    def generate_report(self, results: List[AgentTestResult]) -> str:
